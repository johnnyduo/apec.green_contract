...
bytes32 constant INSURANCE_DETAIL = keccak256("apecgreen.detail");
bytes32 constant INSURANCE_VALIDATION = keccak256("apecgreen.validation");

error Forbidden();

interface IApecResolver {
  ...
  function setApecKyc(bytes32 node, bytes32 kycHash) external;
  function apecKyc(bytes32 node) external view returns(bytes32);
}

contract ApecRegistrarController is Ownable, ERC1155Holder {
  mapping(address => bool) public isValidator;

  address immutable operator;
  bytes32 public immutable rootNode;
  INameWrapper public immutable nameWrapper;
  IApecResolver public immutable resolver;
  ...
  function register(string calldata name, address owner, uint64 expiration, bytes32 kycHash, bytes calldata signature) public {
    ...
    resolver.setApecKyc(node, kycHash);
